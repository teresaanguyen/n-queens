...
//(row, n, board, check, callback)
window.findSolution = function(row, n, board, check, callback) {
  if (row === n) {
    //do something here?
    return callback();
  }
  for (var i = 0; i < n; i++) {
    ...
    // !board[check]()
    if (!board[check]()) {
      // recursively use findSolution while incrementing n
      var result = findSolution(row + 1, n, board, check, callback);
      if (result) {
        return result;
      }
    }
